...
def transform_landmarks_inverse_with_resampling(landmarks, transformation, size, spacing, max_min_distance=None):
    ...
    if dim == 2:
        displacement_field = np.transpose(sitk_np.sitk_to_np(displacement_field), [1, 0, 2])
        ...
        for i in range(len(transformed_landmarks)):
            ...
            invert_min_distance, transformed_coords = np_image.find_quadratic_subpixel_maximum_in_image(-distances)
        ...
        displacement_field = np.transpose(sitk_np.sitk_to_np(displacement_field), [2, 1, 0, 3])
