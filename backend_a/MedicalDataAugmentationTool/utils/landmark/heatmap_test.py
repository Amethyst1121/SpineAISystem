...
from .. import sitk_image
from .. import sitk_np
from .. import np_image
...
class HeatmapTest(object):
    ...
    def get_transformed_image_sitk(self, prediction_np, reference_sitk=None, output_spacing=None, transformation=None):
        ...
        if transformation is not None:
            predictions_sitk = sitk_image.transform_np_output_to_sitk_input(output_image=prediction_np,
            ...
            predictions_np = np_image.split_by_axis(prediction_np, self.channel_axis)
            predictions_sitk = [sitk_np.np_to_sitk(prediction_np) for prediction_np in predictions_np]
    ...
    def get_multiple_maximum_coordinates(self, image):
        ...
        if self.smoothing_sigma > 0.0:
            ...
            indizes, values = np_image.local_maxima(image_smoothed)
        else:
            indizes, values = np_image.local_maxima(image)
        values_greater = values > self.min_max_value
        values = values[values_greater]
        coords = indizes[values_greater]
        refined_coords = [np_image.refine_coordinate_subpixel(image, c) for c in coords]
    ...
    def get_multiple_maximum_coordinates_min_max_distance(self, image):
        ...
        """
        value_coord_pairs = []
        value, coord = np_image.find_quadratic_subpixel_maximum_in_image(image)
        ...
        image = np_image.draw_sphere(np.copy(image), center=coord, radius=self.min_max_distance, value=0)
        value, coord = np_image.find_quadratic_subpixel_maximum_in_image(image)
        while value > absolute_max_value * self.multiple_min_max_value_factor and value > self.min_max_value:
            value_coord_pairs.append((value, coord))
            image = np_image.draw_sphere(np.copy(image), center=coord, radius=self.min_max_distance, value=0)
            value, coord = np_image.find_quadratic_subpixel_maximum_in_image(image)
        return value_coord_pairs
    ...
    def get_landmark(self, image, transformation=None, reference_sitk=None, output_spacing=None):
        ...
        """
        output_spacing = output_spacing or [1] * image.ndim # [1,1]
        if self.return_multiple_maxima:
            ...
            for value, coord in value_coord_pairs:
                coord = np.flip(coord, axis=0) # (x,y)
                coord *= output_spacing
                coord = transform.transform_coords(coord, transformation)
        ...
        if transformation is not None:
            if self.invert_transformation:
                ...
                transformed_np = sitk_np.sitk_to_np_no_copy(transformed_sitk[0])
                value, coord = np_image.find_maximum_in_image(transformed_np)
            ...
            value, coord = np_image.find_maximum_in_image(image)
            coord = np.flip(coord, axis=0)
