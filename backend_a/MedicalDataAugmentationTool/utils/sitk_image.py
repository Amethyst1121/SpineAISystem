...
def argmax(images):
    ...
    images_np = [sitk_np.sitk_to_np_no_copy(image) for image in images]
    # stack along first axis
    image_np = np.stack(images_np, axis=0)
    max_index_np = np_image.argmax(image_np, axis=0, dtype=np.uint8)
    # convert back to sitk, and copy image information of first image
    max_index = sitk_np.np_to_sitk(max_index_np)
    max_index.CopyInformation(images[0])
    return max_index


def split_label_image(image, labels):
    splits_np = np_image.split_label_image(sitk_np.sitk_to_np_no_copy(image), labels)
    splits = []
    for split_np in splits_np:
        split = sitk_np.np_to_sitk(split_np)
...
def merge_label_images(images, labels):
    images_np = [sitk_np.sitk_to_np_no_copy(image) for image in images]
    merged_np = np_image.merge_label_images(images_np, labels)
    merged = sitk_np.np_to_sitk(merged_np)
...
def transform_single_np_output_to_input(output_image, output_spacing, input_image_size, input_image_spacing, input_image_origin, input_image_direction, transform, interpolator='linear', output_pixel_type=None):
    output_image_sitk = sitk_np.np_to_sitk(output_image)
...
def transform_np_output_to_input(output_image, output_spacing, channel_axis, input_image_size, input_image_spacing, input_image_origin, input_image_direction, transform, interpolator='linear', output_pixel_type=None):
    if channel_axis is not None:
        output_images = np_image.split_by_axis(output_image, axis=channel_axis)
    else:
...
def apply_np_image_function(image, f):
    image_np = sitk_np.sitk_to_np(image)
    output_np = f(image_np)
    output = sitk_np.np_to_sitk(output_np)
    copy_information(image, output)
    return output

def hausdorff_distances(image_0, image_1, labels, multi_label=False):
    if multi_label:
        label_images_0 = split_vector_components(image_0)
        label_images_1 = split_vector_components(image_1)
    else:
        label_images_0 = split_label_image(image_0, labels)
        label_images_1 = split_label_image(image_1, labels)
