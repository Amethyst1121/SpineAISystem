...
from .common import create_directories_for_file_name
from ...transformations.intensity.np.normalize import scale_min_max, scale
from ..sitk_image import label_to_rgb, set_spacing_origin_direction
...
def write_png(img, path):
    """
    将 2D sitk 图像保存为 png 格式。
    :param img: 2D sitk.Image 对象
    ...
    # 如果是 3D (z>1)，取一个切片
    if img.GetDepth() > 1:
        print("Warning: 图像是3D的，只保存第0层切片。")
        img = img[:,:,0]

    # 转成 8bit，避免 png 保存不了 16/32 位
    img_8u = sitk.Cast(sitk.RescaleIntensity(img), sitk.sitkUInt8)

    # 直接保存为 png
    sitk.WriteImage(img_8u, path, True)
